...
contract HumanResourcesTest is Test {
    ...
    function test_getEmployeeInfo_unregistered() public view {
        (uint256 weeklySalary, uint256 employedSince, uint256 terminatedAt) = humanResources.getEmployeeInfo(cora); //a unregistered address
    ...
    function test_salaryAvailable_eth() public {
        ...
        humanResources.salaryAvailable(alice), expectedSalary, 0.01e18);
        skip(5 days);
    ...
    function test_reregisterEmployee() public {
        ...
        vm.prank(alice);
        humanResources.withdrawSalary();
        uint256 expectedSalary = ((aliceSalary * 2) / 7) + ((aliceSalary * 2 * 5) / 7); //salary should should include previous unclaim
